...
public class MyRecyLikeAdapter extends RecyclerView.Adapter<MyRecyLikeAdapter.ViewHolder> {

    private Context mContext;
    private List<LikeBean> dbModels;
    private String type;// 0-观看记录 1-离线缓存 2-我的喜欢

    public MyRecyLikeAdapter(Context mContext, List<LikeBean> dbModels, String type) {
    ...
    public void setDbModels(List<LikeBean> dbModels) {
        if (dbModels == null) {
    ...
    @Override
    public MyRecyLikeAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_my_recy, parent, false);
        MyRecyLikeAdapter.ViewHolder hol = new MyRecyLikeAdapter.ViewHolder(view);
        return hol;
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        final LikeBean model = dbModels.get(position);
        if (model != null) {
