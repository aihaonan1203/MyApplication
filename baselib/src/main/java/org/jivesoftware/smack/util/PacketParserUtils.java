...
public class PacketParserUtils {
            ...
            Class<?> objectClass, XmlPullParser parser) throws Exception
    {
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                setFieldValue(object, name, stringValue);
            }
            else if (eventType == XmlPullParser.END_TAG) {
                if (parser.getName().equals(elementName)) {
                    done = true;
    ...
    public static void setFieldValue(Object target, String fName, Object fValue)
        ...
        if(target == null || TextUtils.isEmpty(fName))
        {
            return;
        }
        if(fName.endsWith("[]"))
        ...
        Field field = target.getClass().getDeclaredField(fName);
        Class fType = field.getType();
        try
        {
            Class clazz = target.getClass();
            Method method = clazz.getMethod("set"
                    + upperFirstChar(fName), fType);
